...
import { X, CheckCircle, Clock, Info, Layers, AlertCircle, HelpCircle, Ban, LogIn, LogOut, Users } from 'lucide-react'; 
...
const ToastAlert = ({ title, message, type, onClose }) => {
    ...
    return (
        <div className={`fixed top-5 right-5 z-[2000] flex items-start gap-3 px-4 py-3 rounded-xl shadow-2xl border animate-slideIn bg-white max-w-sm 
            ${type === 'success' ? 'border-orange-500 text-orange-900' : 'border-red-500 text-red-900'}`}>
...
/* ==========================================
   1. SAFETY CONFIRMATION MODAL (For Status)
   ========================================== */
        ...
        <div className="fixed inset-0 z-[2100] flex items-center justify-center bg-black/30 backdrop-blur-sm p-4 animate-fadeIn">
            <div className="bg-white p-6 rounded-2xl shadow-2xl max-w-sm w-full transform scale-100 transition-all">
                ...
                <div className="flex gap-3">
                    ...
                    <button onClick={onConfirm} disabled={loading} className={`flex-1 py-3 text-white rounded-xl font-bold shadow-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${btnColor}`}>
...
/* ==========================================
   2. CHECKOUT CONFIRM MODAL (For Booking)
   ========================================== */
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/30 backdrop-blur-sm p-4 animate-fadeIn">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
                <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                    ...
                    <button onClick={onClose} disabled={loading} className="p-1 hover:bg-gray-200 rounded-full disabled:opacity-50"><X size={20}/></button>
                </div>
                <div className="p-6">
                    ...
                    <div>
                        ...
                        <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar">
                            <div className="flex justify-between text-sm p-2 bg-orange-50 rounded border border-orange-100">
                                ...
                                <span className="font-bold text-gray-900">{formData.numGuest} Pax</span>
                            </div>
                            {cart.map((item, idx) => (
                ...
                <div className="p-5 border-t border-gray-100 bg-gray-50 flex gap-3">
                    <button onClick={onClose} disabled={loading} className="flex-1 py-3 border border-gray-300 rounded-xl font-bold text-gray-600 hover:bg-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Cancel</button>
                    <button onClick={onConfirm} disabled={loading} className="flex-1 py-3 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 shadow-lg shadow-orange-200 transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                        {loading && <span className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"></span>}
...
/* ==========================================
   4. EXTEND MODAL (This is what you showed in screenshot)
   ========================================== */
        ...
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/30 backdrop-blur-sm p-4 animate-fadeIn">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
                <div className="bg-gradient-to-r from-orange-600 to-amber-600 p-6 text-white flex justify-between items-center">
                    ...
                    <button onClick={onClose} disabled={loading} className="p-2 bg-white/20 hover:bg-white/30 rounded-full transition disabled:opacity-50"><X size={20}/></button>
                </div>
                <div className="p-6 overflow-y-auto space-y-6">
                    <div className="flex items-center justify-between border-2 border-gray-100 rounded-2xl p-4 bg-gray-50/50">
                        ...
                        <div className="flex items-center gap-2 bg-white p-1 rounded-xl shadow-sm border border-gray-200">
                            <button onClick={() => setExtendValue(Math.max(1, extendValue - 1))} disabled={loading} className="w-10 h-10 rounded-lg hover:bg-gray-100 flex items-center justify-center font-bold text-lg text-gray-600 transition-colors disabled:opacity-50">-</button>
                            <input type="number" min="1" value={extendValue} onChange={handleValueChange} disabled={loading} className="w-14 h-10 text-center text-2xl font-bold text-orange-600 bg-transparent outline-none disabled:opacity-50"/>
                            <button onClick={() => setExtendValue(extendValue + 1)} disabled={loading} className="w-10 h-10 rounded-lg bg-orange-100 text-orange-600 hover:bg-orange-200 flex items-center justify-center font-bold text-lg transition-colors disabled:opacity-50">+</button>
                        </div>
                    </div>
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50 flex gap-3">
                    <button onClick={onClose} disabled={loading} className="flex-1 py-3 font-bold text-gray-600 bg-white border border-gray-300 rounded-xl hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">Cancel</button>
                    <button onClick={onConfirm} disabled={loading || additionalAmount <= 0} className="flex-1 py-3 font-bold text-white bg-gradient-to-r from-orange-600 to-amber-600 rounded-xl hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2">
                         {loading && <span className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"></span>}
...
const DetailsModal = ({ show, onClose, transaction, viewType }) => {
    ...
    const guestCount = transaction.num_guest || transaction.numGuest || 0;
