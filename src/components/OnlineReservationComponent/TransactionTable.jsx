...
const ToastAlert = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`fixed top-5 right-5 z-[2000] flex items-center gap-3 px-4 py-3 rounded-xl shadow-2xl border animate-slideIn ${
      type === 'success' 
...
const TransactionTable = ({ 
  ...
  const closeAllModals = () => {
    setModals({ mobile: false, checkIn: false, extend: false, proof: false, detail: false, action: false });
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => fetchTransactions(true), 3000);
  ...
  const processCheckIn = async () => {
    ...
    setProcessLoading(true);
    try {
      await api.put(`/api/transactions/${selectedTransaction.id}/status`, { 
        booking_status: 'Checked-In', 
        balance: 0, 
        payment_status: 'Fully Paid' 
      });
      
      showAlert('success', `Check-in successful for ${selectedTransaction.customer_name}!`);
      await fetchTransactions(true); 
      closeAllModals();
    } catch (error) {
      console.error(error);
      showAlert('error', 'Failed to check-in customer.');
    } finally {
      setProcessLoading(false);
    }
  };
  ...
  const processExtend = async (extensionData) => {
    ...
    try {
      ...
      showAlert('success', `Extended successfully! Fee: ₱${extensionData.additionalAmount}`);
      await fetchTransactions(true);
      closeAllModals();
    } catch (error) {
      console.error(error);
  ...
  const processPaymentAction = async (action) => {
    if (!selectedTransaction) return;
    await handleStatusUpdate(selectedTransaction.id, action);
  };
  ...
  const filteredTransactions = useMemo(() => {
    ...
    let result = transactions.filter(t => {
      ...
      const matchesSearch = (t.customer_name || '').toLowerCase().includes(q) || 
      ...
      if (viewMode === 'active') {
        ...
        if (!historyStatuses.includes(status)) return false;
      }
    ...
    result.sort((a, b) => {
        ...
        const resA = a.reservations?.[0];
        const resB = b.reservations?.[0];
        
        const checkInA = resA?.check_in_date ? new Date(resA.check_in_date) : createdA;
        const checkInB = resB?.check_in_date ? new Date(resB.check_in_date) : createdB;
        const checkOutA = resA?.check_out_date ? new Date(resA.check_out_date) : createdA;
        const checkOutB = resB?.check_out_date ? new Date(resB.check_out_date) : createdB;

        if (statusFilter === 'Pending') return createdA - createdB; 
        if (statusFilter === 'Confirmed') return checkInA - checkInB; 
        if (statusFilter === 'Checked-In') return checkOutA - checkOutB; 
    ...
    <>
      ...
      <div className="hidden lg:block bg-white border border-gray-200 rounded-xl shadow-sm mt-4 overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg text-xs font-medium border border-blue-100">
              <Clock size={14}/> 
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-orange-50/50 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        ...
                        <button onClick={() => openDetailModal(t, 'amenities')} className="text-[11px] text-blue-600 hover:text-blue-800 flex items-center gap-1 mt-1 font-semibold" type="button">
                      ...
                      <td className="px-4 py-4">
                        {(t.extensions || []).length > 0 ? (
                          <div className="flex flex-col items-start">
                            <span className="text-[10px] font-bold text-purple-700 bg-purple-50 px-1.5 py-0.5 rounded border border-purple-100">{t.extensions.length} added</span>
                            <button onClick={() => openDetailModal(t, 'extensions')} className="text-[11px] text-purple-500 hover:text-purple-700 flex items-center gap-1 mt-1 font-semibold" type="button">
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex flex-col items-center text-xs">
                          <span className="text-gray-500">{inDT.date} <b className="text-green-700">{inDT.time}</b></span>
                          <span className="text-gray-300 my-0.5">↓</span>
                          <span className="text-gray-500">{outDT.date} <b className="text-orange-700">{outDT.time}</b></span>
                        </div>
                      </td>
                      <td className="px-4 py-4 text-center">
                        <div className="flex flex-col items-center">
                          <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border mb-1 ${['Checked-In', 'Completed'].includes(t.booking_status) ? 'bg-green-100 text-green-700 border-green-200' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>
                      ...
                      <td className="px-4 py-4 text-center">
                        ...
                        }`}>{t.booking_status}</span>
                      </td>
                      <td className="px-4 py-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          {t.proof_of_payment && (
                            <button onClick={() => openProofModal(t)} className="flex items-center gap-1 pl-1.5 pr-2 py-1.5 bg-cyan-50 text-cyan-700 border border-cyan-200 rounded-lg hover:bg-cyan-100 transition-colors shadow-sm" title="Check Payment" type="button">
                              <Eye size={14}/><span className="text-[11px] font-bold uppercase">Proof</span>
                            </button>
                          )}
                          
                          {t.booking_status === 'Confirmed' && (
                            <>
                              <button onClick={() => openCheckInModal(t)} className="p-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors" title="Check In" type="button">
                                <LogIn size={16}/>
                              </button>
                              <button onClick={() => openActionModal('Cancelled', t)} className="p-2 bg-gray-100 text-gray-500 rounded-lg hover:bg-red-100 hover:text-red-600 transition-colors" title="Cancel" type="button">
                                <Ban size={16}/>
                              </button>
                            </>
                          )}
                          ...
                          {t.booking_status === 'Checked-In' && (
                            <>
                              <button onClick={() => openExtendModal(t)} className="p-2 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200" title="Extend" type="button">
                                <Clock size={16}/>
                              </button>
                              <button onClick={() => openActionModal('Completed', t)} className="p-2 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200" title="Check Out" type="button">
                                <LogOut size={16}/>
                              </button>
                            </>
                          )}
                          ...
                          {t.booking_status === 'Pending' && !t.proof_of_payment && (
                             <button onClick={() => openActionModal('Cancelled', t)} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100" title="Reject" type="button">
                                <Ban size={16}/>
                             </button>
                          )}
                        </div>
      ...
      <div className="lg:hidden bg-white border border-gray-200 rounded-xl shadow-sm mt-4 overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-gray-50">
          <div className="text-sm text-gray-600 mb-2">Showing {filteredTransactions.length} results</div>
        </div>
        <div className="divide-y divide-gray-100">
          {filteredTransactions.length === 0 ? (
            <div className="px-6 py-12 text-center text-gray-400 italic">No transactions found.</div>
          ) : (
            filteredTransactions.map(t => (
                <div key={t.id} onClick={() => openMobileModal(t)} className="p-4 hover:bg-orange-50 cursor-pointer">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <div className="font-mono text-xs font-bold text-gray-600 mb-1">{t.transaction_ref}</div>
                      <h3 className="font-bold text-gray-800 text-lg">{t.customer_name}</h3>
                    </div>
                    {/* ✅ UPDATED STATUS COLORS (Mobile) */}
                  ...
                  <div className="flex justify-between items-center text-sm text-gray-500">
                    <span>₱{parseFloat(t.total_amount).toLocaleString()}</span>
      ...
      <ProofModal 
        ...
        loading={processLoading}
        onClose={closeAllModals}
        onAction={processPaymentAction} 
      />

      {/* IMPORTANT: onClose uses closeDetailOnly here */}
      <DetailModal 
      ...
      <ActionModal
        ...
        loading={processLoading}
        onClose={closeAllModals}
        onConfirm={() => processPaymentAction(actionType)}
      />
    </>
  );
};
